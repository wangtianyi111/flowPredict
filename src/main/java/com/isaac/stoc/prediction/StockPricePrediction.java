package com.isaac.stoc.prediction;
...
public class StockPricePrediction {
	...
	private static int exampleLength = 58; // time series length, assume 22 working days per month

	public static void main(String[] args) throws IOException {
		String file = new ClassPathResource("rmgc-lzj.csv").getFile().getAbsolutePath();
		String symbol = "lzj"; // stock name
		int batchSize = 64; // mini-batch size
		double splitRatio = 0.9; // 90% for training, 10% for testing
		int epochs = 150; // training epochs,这个100是什么意思!!!

		log.info("Create dataSet iterator...");
		PriceCategory category = PriceCategory.OUT; // CLOSE: predict close price
		StockDataSetIterator iterator = new StockDataSetIterator(file, symbol, batchSize, exampleLength, splitRatio,category);
		System.out.println("iterator="+iterator.totalExamples());
		System.out.println("iterator="+iterator.totalOutcomes());
		...
		File locationToSave = new File(
				"src/main/resources/StockPriceLSTM_".concat(String.valueOf(category)).concat(".zip"));
	...
	private static void predictPriceOneAhead(MultiLayerNetwork net, List<Pair<INDArray, INDArray>> testData, double max,
		...
		System.out.println("actuals.length="+actuals.length);
		double error = 0.0;
		int count = 0;
		for (int i = 0; i < predicts.length; i++) {
			log.info(predicts[i] + "," + actuals[i]);
			double pre = predicts[i];
			double act = actuals[i];
			if (act > 50) {
				error += Math.abs((pre - act)) / act;
				count ++;
			}
		}
		log.info("error=" + error/count);	
		log.info("Plot...");
